...
const Announcements = () => {
  ...
  const handleDelete=async()=>{
    try {
      await axios.delete(`http://localhost:5000/api/announcement/delete/${announcementId}`)
    } catch (error) {
      console.log(error)
    }
  }
  ...
  const saveAnnouncement=async(data)=>{
    try {
      await axios.post(`http://localhost:5000/api/announcement/edit/${announcementId}`, data)
    } catch (error) {
      console.log(error)
    }
  }
